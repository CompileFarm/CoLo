 ...
 *
 * $Id: udp.c 186 2006-01-17 23:03:58Z pdh $
 *
...
void udp_in(struct frame *frame)
{
  ...
  for(indx = 0;; ++indx)
	...
	return;
      if(socks[indx].inuse && socks[indx].port == port && (!socks[indx].peer_port || (socks[indx].peer_port == frame->udp_src && socks[indx].peer_ip == frame->ip_src)))
	{
	  break;
	}
    }
